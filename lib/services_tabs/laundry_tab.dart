...
class ListViewActivity extends State<LaundryTab> {
...
//                future: getData(),
            ...
            child: LayoutBuilder(builder:
              ...
              debugPrint(_currentItem + ">>>>" + lstLaundryData.toString());
...
//                    padding: const EdgeInsets.all(20.0),
                          padding: EdgeInsets.only(
                              left: 0.0, right: 0.0, top: 2.0, bottom: 4.0),
...
//                                        lstLaundryData[index]['svc_noitemselected']=lstNoOfItemsSelected[index];
                            ...
                            return Card(
                                ...
                                child: ListTile(
                                  title: Text(
                                    lstLaundryData[index]['svc_article'],
                                    style: TextStyle(
                                        color: Colors.black54,
                                        fontWeight: FontWeight.w700,
                                        fontSize: 18,
                                  ...
                                  subtitle: Text(
                                    "Category: " +
                                        ...
                                        ['svc_demographic'] +
                                        "\nPrice: KSh " +
                                        lstLaundryData[index]['svc_price']
                                            .toString(),
                                    style: TextStyle(
                                      fontSize: 14,
                                      ...
                                      Column(
                                          ...
                                          children: <Widget>[
                                            ...
                                            SizedBox(
                                              ...
                                              width: 20,
                                              child: Text(
                                                lstLaundryData[index]['svc_noitemselected'].toString(),
                                                //lstNoOfItemsSelected[index].toString(),
                                                textAlign: TextAlign.center,
                                                style: TextStyle(
                                                    color: Colors.black,
                                                    fontSize: 18,
                                          ...
                                          onPressed: () {
                                            for (int x=0;x<lstRawData.length;x++){
                                              if (lstLaundryData[index]['svc_id']==lstRawData[x]['svc_id']) {
                                                lstRawData[x]['svc_noitemselected']++;
                                                setState(() =>
                                                strNoCounter=lstRawData[x]['svc_noitemselected'].toString()
                                                );
                                                debugPrint("item added>>>>" +
                                                    lstRawData[x]['svc_article']+"|"+lstRawData[x]['svc_noitemselected'].toString());
                                                break;
                                              }
                                            }
                                            ...
                                            //21) {
                                            debugPrint("Selected added>>>>" +
                                                lstRawData[index]['svc_article'].toString());
                                            //}
                                          }
...
// Create a new marker
    ...
    for (int i=0;i<lstRawData.length;i++) {
      if (lstRawData[i]['svc_noitemselected'] > 0) {
        ...
        debugPrint("srinvoice>>>>" +
            lstRawData[i]['svc_article'].toString());
        intTotalPrice=intTotalPrice+lstRawData[i]['svc_noitemselected']*lstRawData[i]['svc_price'];
      }
    }
    ...
    Future<String> senddata() async {
      ...
      if (strLocAddress.isEmpty) {
         Flushbar(
          ...
          message: "Please click on the location icon to select a location pin from the map",
          duration: Duration(seconds: 3),
          isDismissible: false,
        )
          ..show(context);
      ...
      if (strHouseNameNo.text.length<3 || strHouseNameNo.text.isEmpty || strHouseNameNo.text =="") {
        Flushbar(
      ...
      if (concatenate.toString().isNotEmpty && concatenate.toString() !="" && strHouseNameNo.text.length>3 && strHouseNameNo.text.isNotEmpty && strHouseNameNo.text !="" && strLocAddress.isNotEmpty){
        ...
        if (strResponse.body.toString() == "ErrorRE101") {
          ...
          Flushbar(
            title: "Ticket",
            message: "Your ticket has been created successfully. You can find it in MyTickets tab",
            duration: Duration(seconds: 3),
            isDismissible: false,
          )..show(context);
                ...
                children: [
                  ...
                  Column(
                      ...
                      children: <Widget>[
                        ...
                        //////////////////////
                        ListTile(
                          title: Text("Location Address: "+strLocAddress,
                            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
                          ),

                          trailing: Icon(Icons.location_on_outlined,size: 30.0,color:Colors.lightBlue),
                          onTap:getpin,
                        ),
                        SizedBox(
                          height: 20,),
...
//                           ),
                        ...
                        //////////////////////
                        ListTile(
                          title: Text("Start time (24hr system): ${time.hour}:${time.minute}",
                            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
                          ),

                          trailing: Icon(Icons.more_time_outlined,size: 30.0,color:Colors.lightBlue),
                        ...
                        ListTile(
                          title: Text(
                            "Start"+" date: "+ "${selectedDate.toLocal()}".split(' ')[0],
                            style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
                          ),
                          trailing: Icon(Icons.date_range_outlined,size: 30.0,color:Colors.lightBlue),
